...
def doings(shit):
    ...
    if 'help' in do:
        print(
            '         Help\n         I want to play\n         Find me something(for example: House Photo, Meme or Street Image, etc.)\n         I want to open calculator\n         I want to jerk off\n         I want to sex\n         I want to be hacker\\cheater\n         ')
    elif 'fuck' in do or 'sex' in do or 'porn' in do:
        print('Ye! Fuck me! Ye! Omg! aaa~')
        time.sleep(3)
        print('Thanks, i\'m cum')
        time.sleep(2)
    elif 'jerk off' in do:
        ...
        while action not in {'+', '-', '\\', '*'}:
            ...
            print(eval(f'{what_number1}{action}{what_number2}'))
    elif 'find' in do:
        find_req = do.replace('find ', '').replace('me ', '').replace(' pls', '').replace(' please', '')
